...
| 06-02-01 | 02 | 2 | INT-01 / INT-03 | T-06-01 / T-06-02 / T-06-04 | PostgreSQL schema enforces tenant scoping, unique `(tenant_id, source_event_id, topic)`, valid statuses, positive attempts, and due-time fields. | integration | `cargo test -p es-store-postgres --test outbox outbox_ -- --test-threads=1 --nocapture` | No - Wave 0 | pending |
| 06-03-01 | 03 | 3 | INT-01 | T-06-01 / T-06-04 / T-06-05 | Append transaction creates event rows, command dedupe, and derived outbox rows atomically; conflicts, rollback, and duplicate idempotency replay do not create duplicate outbox rows. | integration | `cargo test -p es-store-postgres --test outbox append_ -- --test-threads=1 --nocapture` | No - Wave 0 | pending |
| 06-04-01 | 04 | 4 | INT-02 / INT-03 | T-06-02 / T-06-03 / T-06-06 | Dispatcher claims pending rows with bounded batches, publishes through `Publisher`, marks success, schedules retry or failed status on failure, and preserves deterministic idempotency keys. | unit + integration | `cargo test -p es-outbox dispatcher -- --nocapture && cargo test -p es-store-postgres --test outbox dispatcher_ -- --test-threads=1 --nocapture` | No - Wave 0 | pending |
| 06-05-01 | 05 | 5 | INT-04 | T-06-03 / T-06-07 / T-06-08 | Process manager reads committed order/product events by tenant/global position, submits follow-up commands through command gateways, and advances durable PostgreSQL offsets only after replies finish or events are intentionally skipped. | unit + integration | `cargo test -p es-outbox process_manager -- --nocapture && cargo test -p es-store-postgres --test outbox process_manager_advances_postgres_offset_after_gateway_replies -- --test-threads=1 --nocapture` | No - Wave 0 | pending |
