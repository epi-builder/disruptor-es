...
use es_core::{StreamId, StreamRevision};
use es_runtime::{CommandOutcome, CommittedAppend, CommandGateway, PartitionRouter, RuntimeError};
use example_commerce::{Order, OrderCommand, OrderId, OrderReply};
use tower::ServiceExt;
use uuid::Uuid;

#[tokio::test]
async fn place_order_submits_command_and_returns_response_contract() {
    ...
    let (product_gateway, _product_rx) =
        CommandGateway::new(PartitionRouter::new(4).expect("router"), 8)
            .expect("product gateway");
...
#[tokio::test]
async fn response_contract_maps_overload_to_json_429() {
    let (order_gateway, _order_rx) =
        CommandGateway::<Order>::new(PartitionRouter::new(1).expect("router"), 1)
            .expect("order gateway");
    let (product_gateway, _product_rx) =
        CommandGateway::new(PartitionRouter::new(1).expect("router"), 1)
            .expect("product gateway");
    let (user_gateway, _user_rx) =
    ...
    let first = place_order_request("idem-overload-1");
    let second = place_order_request("idem-overload-2");

    let accepted = app.clone().oneshot(first).await.expect("first response");
    assert_eq!(StatusCode::SERVICE_UNAVAILABLE, accepted.status());

    let overloaded = app.oneshot(second).await.expect("second response");
...
#[tokio::test]
async fn response_contract_maps_conflict_to_json_409() {
