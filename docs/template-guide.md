...
- Use `shard_count`, `ingress_capacity`, and `ring_size` from each archived JSON file when explaining shard-count or lane-shape differences.
- Use `append_latency_observed`, `append_latency_p95_micros`, `append_latency_sample_count_delta`, and `append_latency_unavailable_reason` together when describing append behavior. If `append_latency_observed` is `false`, keep the label at `durable command path (append latency unavailable in live HTTP artifacts)` or broader.
...
- For hot-key diagnostics, cite `failure_kind_counts` and bounded `sample_failures` rather than only `commands_failed`.

Phase 13.2 closed the old zero-scrape archive gap. The current archive-safe baseline files are `live-http-unique.json`, `live-http-shard-1.json`, and `live-http-shard-8.json` regenerated by `PHASE13_1_COMPARE_MODE=baseline bash scripts/compare-stress-layers.sh`, with the rerun evidence recorded in `13.2-BASELINE-REGEN.md` and `13.2-SCRAPE-REPRO.md`. Those files now satisfy the `metrics_scrape_successes > 0` gate, but they also report `append_latency_observed = false` and `append_latency_unavailable_reason = "missing_histogram_count"`, so the current archive label is `durable command path (append latency unavailable in live HTTP artifacts)`, not a narrower storage-append claim and not `inconclusive`.

Explain the post-Phase-13.2 throughput ceiling in terms of the slowest measured layer. For example, call out durable PostgreSQL append, HTTP overhead, bounded admission, or command-path side effects when they dominate. Do not collapse every shortfall into "the ring is slow" when the slower evidence lane is outside the ring, and do not revive stale pre-Phase-13.2 zero-scrape files once regenerated baseline evidence exists.
